...
func newClient(t *testing.T, uc expense.UseCase) *Client {
...
func Test_handleExpenseCmd(t *testing.T) {
	...
	addExpCall := ucMock.EXPECT().AddExpense(models.UserID(userID), expectedExp).MaxTimes(1).Return(expectedExp, nil).After(msgCall)
	teleCtxMock.EXPECT().Send("Expense successfully created").Times(1).After(addExpCall) // send call

	cl := newClient(t, ucMock)
	err := cl.handleExpenseCmd(teleCtxMock)
	require.NoError(t, err)
}
...
func Test_handleExpensesReportCmd(t *testing.T) {
	...
	reportCall := ucMock.EXPECT().ExpensesSummaryByCategorySince(models.UserID(userID), since, till).Times(1).
		Return(report, nil).After(msgCall)
	teleCtxMock.EXPECT().Send(report.Text()).Times(1).Return(nil).After(reportCall)

	cl := newClient(t, ucMock)
	err := cl.handleExpensesReportCmd(teleCtxMock)
	require.NoError(t, err)
}
...
func Test_handleExpensesListCmd(t *testing.T) {
	...
	reportCall := ucMock.EXPECT().ExpensesAscendSinceTill(models.UserID(userID), since, till, maxExpensesList).Times(1).
	...
	cl := newClient(t, ucMock)
	err := cl.handleExpensesListCmd(teleCtxMock)
	require.NoError(t, err)
}
